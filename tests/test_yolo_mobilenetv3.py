...
class YoloV3MobilenetV3Tests(unittest.TestCase) :
    def test_train(self) :
        yolov3 = YoloV3(backbone='mobilenetv3_large_w1',img_size=416,n_classes=1)
    ...
    def test_eval(self) :
        yolov3 = YoloV3(backbone='mobilenetv3_large_w1',img_size=416,n_classes=1).eval()
        out = yolov3(torch.rand(1,3,416,416))
        self.assertEqual(out.size(),torch.Size([1,3*13*13+3*26*26+3*52*52,6]))
    
    def gpu_test_train(self) :
        device = torch.device('cuda')
        yolov3 = YoloV3(backbone='mobilenetv3_large_w1',img_size=416,n_classes=1).to(device)
    ...
    def gpu_test_eval(self) :
        device = torch.device('cuda')
        yolov3 = YoloV3(backbone='mobilenetv3_large_w1',img_size=416,n_classes=1).eval().to(device)
        out = yolov3(torch.rand(1,3,416,416).to(device))
        self.assertEqual(out.size(),torch.Size([1,3*13*13+3*26*26+3*52*52,6]))