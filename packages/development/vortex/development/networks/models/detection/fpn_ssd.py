from ...modules.postprocess.fpn_ssd import FPNSSDPostProcess
from ...modules.losses.ssd import MultiBoxLoss
...
from ...modules.heads.detection.retinanet import RetinaHead
...
class PriorBox(nn.Module):
    ...
    """
    __constants__ = ['min_dim', 'aspect_ratios', 'variance',
                     'feature_maps', 'min_sizes', 'max_sizes', 'steps', 'clip']
...
class FPNSSD(FPNBackbone):
    ...
    def __init__(self, image_size: int, n_classes: int, aspect_ratios: List[float] = [1.], variance: List[int] = [0.1, 0.2], anchors: Union[Sequence[Sequence[Sequence[int]]]] = None, *args, **kwargs):
        from ...modules.heads.detection.fpn import LastLevelP6P7
        from ...modules.heads.detection.ssd import SSDHead
        super(FPNSSD, self).__init__(extra_block_factory=LastLevelP6P7, *args, **kwargs)
        ...
        assert all(fpn_channels[0] == ch for ch in fpn_channels), "currently only support equal number of channel over feature pyramids"
        # TODO : read extra channels from fpn!
        head_module = SSDHead(fpn_channels[0], n_anchors=n_anchors,n_classes=n_classes+1, extra_channels=[fpn_channels[0], fpn_channels[0]])
        self.add_module('head', head_module)

        # TODO : move detection tensor info to postprocess or decoder (?)
        self.register_buffer('bounding_box_indices',tensor([0, 1, 2, 3]).long())
        self.register_buffer('bounding_box_axis', tensor([1]).long())
        self.register_buffer('classes_indices', tensor([5]).long())
        self.register_buffer('classes_axis', tensor([1]).long())
        self.register_buffer('confidences_indices', tensor([4]).long())
        self.register_buffer('confidences_axis', tensor([1]).long())
        self.output_format = {
            'bounding_box': {
                'indices': self.bounding_box_indices.numpy().tolist(),
                'axis': int(self.bounding_box_axis.numpy()),
            },
            'class_label': {
                'indices': self.classes_indices.numpy().tolist(),
                'axis': int(self.classes_axis.numpy()),
            },
            'class_confidence': {
                'indices': self.confidences_indices.numpy().tolist(),
                'axis': int(self.confidences_axis.numpy()),
            }
        }
    ...
    def forward(self, x: Tensor) -> Union[Tensor, Tuple[Tensor, Tensor]]:
        p3, p4, p5, extra = super(FPNSSD, self).forward(x)
        predictions = self.head(p3, p4, p5, extra)
        return predictions
...
def create_model_components(preprocess_args: EasyDict, network_args: EasyDict, loss_args: EasyDict, postprocess_args: EasyDict) -> EasyDict:
    ...
    network = FPNSSD(**network_args)
    network.head.apply(weights_init)
    loss_args.num_classes = network.head.n_classes
    loss_args.priors = network.default_boxes
    loss_args.variance = network.anchor_gen.variance
    loss_fn = MultiBoxLoss(**loss_args)
    postprocess_args.variance = network.anchor_gen.variance
    postprocess_args.priors = network.default_boxes
    postprocess_args.n_classes = network.head.n_classes
    postprocess = FPNSSDPostProcess(**postprocess_args)
    model_components.network = network
    model_components.loss = loss_fn
    model_components.postprocess = postprocess
    model_components.collate_fn = 'SSDCollate'
