...
model_cfgs = dict(
    regnetx_002=_cfg(w0=24, wa=36.44, wm=2.49, group_w=8, depth=13),
    regnetx_004=_cfg(w0=24, wa=24.48, wm=2.54, group_w=16, depth=22),
    regnetx_006=_cfg(w0=48, wa=36.97, wm=2.24, group_w=24, depth=16),
    regnetx_008=_cfg(w0=56, wa=35.73, wm=2.28, group_w=16, depth=16),
    regnetx_016=_cfg(w0=80, wa=34.01, wm=2.25, group_w=24, depth=18),
    regnetx_032=_cfg(w0=88, wa=26.31, wm=2.25, group_w=48, depth=25),
    regnetx_040=_cfg(w0=96, wa=38.65, wm=2.43, group_w=40, depth=23),
    regnetx_064=_cfg(w0=184, wa=60.83, wm=2.07, group_w=56, depth=17),
    regnetx_080=_cfg(w0=80, wa=49.56, wm=2.88, group_w=120, depth=23),
    regnetx_120=_cfg(w0=168, wa=73.36, wm=2.37, group_w=112, depth=19),
    regnetx_160=_cfg(w0=216, wa=55.59, wm=2.1, group_w=128, depth=22),
    regnetx_320=_cfg(w0=320, wa=69.86, wm=2.0, group_w=168, depth=23),
    regnety_002=_cfg(w0=24, wa=36.44, wm=2.49, group_w=8, depth=13, se_ratio=0.25),
    regnety_004=_cfg(w0=48, wa=27.89, wm=2.09, group_w=8, depth=16, se_ratio=0.25),
    regnety_006=_cfg(w0=48, wa=32.54, wm=2.32, group_w=16, depth=15, se_ratio=0.25),
    regnety_008=_cfg(w0=56, wa=38.84, wm=2.4, group_w=16, depth=14, se_ratio=0.25),
    regnety_016=_cfg(w0=48, wa=20.71, wm=2.65, group_w=24, depth=27, se_ratio=0.25),
    regnety_032=_cfg(w0=80, wa=42.63, wm=2.66, group_w=24, depth=21, se_ratio=0.25),
    regnety_040=_cfg(w0=96, wa=31.41, wm=2.24, group_w=64, depth=22, se_ratio=0.25),
    regnety_064=_cfg(w0=112, wa=33.22, wm=2.27, group_w=72, depth=25, se_ratio=0.25),
    regnety_080=_cfg(w0=192, wa=76.82, wm=2.19, group_w=56, depth=17, se_ratio=0.25),
    regnety_120=_cfg(w0=168, wa=73.36, wm=2.37, group_w=112, depth=19, se_ratio=0.25),
    regnety_160=_cfg(w0=200, wa=106.23, wm=2.48, group_w=112, depth=18, se_ratio=0.25),
    regnety_320=_cfg(w0=232, wa=115.89, wm=2.53, group_w=232, depth=20, se_ratio=0.25),
)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_chs, out_chs, stride=1, dilation=1, bottleneck_ratio=1, group_width=1, se_ratio=0.25,
                 downsample=None, act_layer=nn.ReLU, norm_layer=nn.BatchNorm2d, norm_kwargs=None, aa_layer=None,
        ...
        cargs = dict(act_layer=act_layer, norm_layer=norm_layer, norm_kwargs=norm_kwargs, 
            aa_layer=aa_layer, drop_block=drop_block)
        self.conv1 = ConvBnAct(in_chs, bottleneck_chs, kernel_size=1, **cargs)
        self.conv2 = ConvBnAct(bottleneck_chs, bottleneck_chs, kernel_size=3, stride=stride, 
            dilation=dilation, groups=groups, **cargs)
        if se_ratio:
...
def downsample_conv(in_chs, out_chs, kernel_size, stride=1, dilation=1, norm_layer=None, norm_kwargs=None):
    norm_layer = norm_layer or nn.BatchNorm2d
    norm_kwargs = norm_kwargs or {}
    kernel_size = 1 if stride == 1 and dilation == 1 else kernel_size
    dilation = dilation if kernel_size > 1 else 1
    return ConvBnAct(in_chs, out_chs, kernel_size, stride=stride, dilation=dilation, 
        norm_layer=norm_layer, norm_kwargs=norm_kwargs, act_layer=None)


def downsample_avg(in_chs, out_chs, kernel_size, stride=1, dilation=1, norm_layer=None, norm_kwargs=None):
    ...
    return nn.Sequential(pool, ConvBnAct(in_chs, out_chs, 1, stride=1, norm_layer=norm_layer, 
        norm_kwargs=norm_kwargs, act_layer=None))
...
class RegStage(nn.Module):
    ...
    def __init__(self, in_chs, out_chs, stride, dilation, depth, bottle_ratio, group_width,
        ...
        norm_kwargs = norm_kwargs or {}
         ...
         # FIXME setup to pass various aa, norm, act layer common args
        block_kwargs = dict(norm_layer=norm_layer, norm_kwargs=norm_kwargs) 
...
class RegNet(nn.Module):
    ...
    def __init__(self, cfg, in_chans=3, num_classes=1000, output_stride=32, drop_path_rate=0.,
        ...
        self.stem = ConvBnAct(in_chans, stem_width, 3, stride=2, norm_layer=norm_layer, norm_kwargs=norm_kwargs)

        self.out_channels = [stem_width]
        ...
        for i, stage_args in enumerate(stage_params):
            ...
            self.add_module(stage_name, RegStage(prev_width, **stage_args, norm_layer=norm_layer, 
                norm_kwargs=norm_kwargs, se_ratio=se_ratio))
        ...
        self.head = ClassifierHead(in_chs=prev_width, num_classes=num_classes, drop_rate=drop_rate)
    ...
    def _get_stage_params(self, cfg, default_stride=2, output_stride=32, drop_path_rate=0.):
        ...
        stage_params = [
            ...
            zip(stage_widths, stage_strides, stage_dilations, stage_depths, 
                stage_bottle_ratios, stage_groups,stage_dpr)
        ]
        return stage_params
...
def regnetx_002(pretrained=False, progress=True, **kwargs):
    """RegNetX-200MF"""
    return _regnet('regnetx_002', pretrained, progress, **kwargs)

def regnetx_004(pretrained=False, progress=True, **kwargs):
    """RegNetX-400MF"""
    return _regnet('regnetx_004', pretrained, progress, **kwargs)

def regnetx_006(pretrained=False, progress=True, **kwargs):
    """RegNetX-600MF"""
    return _regnet('regnetx_006', pretrained, progress, **kwargs)

def regnetx_008(pretrained=False, progress=True, **kwargs):
    """RegNetX-800MF"""
    return _regnet('regnetx_008', pretrained, progress, **kwargs)

def regnetx_016(pretrained=False, progress=True, **kwargs):
    """RegNetX-1.6GF"""
    return _regnet('regnetx_016', pretrained, progress, **kwargs)

def regnetx_032(pretrained=False, progress=True, **kwargs):
    """RegNetX-3.2GF"""
    return _regnet('regnetx_032', pretrained, progress, **kwargs)

def regnetx_040(pretrained=False, progress=True, **kwargs):
    """RegNetX-4.0GF"""
    return _regnet('regnetx_040', pretrained, progress, **kwargs)

def regnetx_064(pretrained=False, progress=True, **kwargs):
    """RegNetX-6.4GF"""
    return _regnet('regnetx_064', pretrained, progress, **kwargs)

def regnetx_080(pretrained=False, progress=True, **kwargs):
    """RegNetX-8.0GF"""
    return _regnet('regnetx_080', pretrained, progress, **kwargs)

def regnetx_120(pretrained=False, progress=True, **kwargs):
    """RegNetX-12GF"""
    return _regnet('regnetx_120', pretrained, progress, **kwargs)

def regnetx_160(pretrained=False, progress=True, **kwargs):
    """RegNetX-16GF"""
    return _regnet('regnetx_160', pretrained, progress, **kwargs)

def regnetx_320(pretrained=False, progress=True, **kwargs):
    """RegNetX-32GF"""
    return _regnet('regnetx_320', pretrained, progress, **kwargs)

def regnety_002(pretrained=False, progress=True, **kwargs):
    """RegNetY-200MF"""
    return _regnet('regnety_002', pretrained, progress, **kwargs)

def regnety_004(pretrained=False, progress=True, **kwargs):
    """RegNetY-400MF"""
    return _regnet('regnety_004', pretrained, progress, **kwargs)

def regnety_006(pretrained=False, progress=True, **kwargs):
    """RegNetY-600MF"""
    return _regnet('regnety_006', pretrained, progress, **kwargs)

def regnety_008(pretrained=False, progress=True, **kwargs):
    """RegNetY-800MF"""
    return _regnet('regnety_008', pretrained, progress, **kwargs)

def regnety_016(pretrained=False, progress=True, **kwargs):
    """RegNetY-1.6GF"""
    return _regnet('regnety_016', pretrained, progress, **kwargs)

def regnety_032(pretrained=False, progress=True, **kwargs):
    """RegNetY-3.2GF"""
    return _regnet('regnety_032', pretrained, progress, **kwargs)

def regnety_040(pretrained=False, progress=True, **kwargs):
    """RegNetY-4.0GF"""
    return _regnet('regnety_040', pretrained, progress, **kwargs)

def regnety_064(pretrained=False, progress=True, **kwargs):
    """RegNetY-6.4GF"""
    return _regnet('regnety_064', pretrained, progress, **kwargs)

def regnety_080(pretrained=False, progress=True, **kwargs):
    """RegNetY-8.0GF"""
    return _regnet('regnety_080', pretrained, progress, **kwargs)

def regnety_120(pretrained=False, progress=True, **kwargs):
    """RegNetY-12GF"""
    return _regnet('regnety_120', pretrained, progress, **kwargs)

def regnety_160(pretrained=False, progress=True, **kwargs):
    """RegNetY-16GF"""
    return _regnet('regnety_160', pretrained, progress, **kwargs)
